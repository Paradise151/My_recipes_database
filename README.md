...
## **Часть 2**  
Таблицы **духовка** (3), **мультиварка** (5), **плита** (4), **микроволновка** (6), **холодная готовка** (7) одинаковы по строению - содержат первичный ключ и название режима (name_mode). Таблица **шаги готовки** (2) на первичные ключи вышеописанных таблиц средств готовки ссылается (4 внешних ключа) и содержит первичный ключ - (step_id). Таблица авторов помимо первичного ключа включает ещё и имя автора -(name_author). **Таблица рецептов** (9) ссылается на первичный ключ **таблицы авторов** (11) и ещё имеет следующие атрибуты: название рецепта (name_recipe), число порций (number_of_servings), первичный ключ (recipe_id) и комментарий к рецепту (discription). Таблица **тип ингредиентов** (8) имеет первичный ключ и название типа - (name_ingredient_type). Таблица **ингрдиенты** (1) ссылается на первичный ключ таблицы типы ингрдиентов и включает название ингредиента (name_ingredient). И напоследок, таблица шагов рецептов ссылается на первичные ключи трёх таблиц - ингредиенты, шаги готовки и рецепты. И, помимо первичного ключа, содержит следующие атрибуты: число частей (может быть как штукой, так и долей , например,  1.5 порции воды на 1 порцию риса), вес в кг (weight_kg), время готовки (cooking_time), температура (temperature_celcius) и давление (специально для мультиварки).  
Таким образом получаем следующую архитектуру базы данных (ER - диаграмму):  
![ER диаграмма базы данных рецептов](main/ER diagrama recipe db.png)
## **Часть 3**  
